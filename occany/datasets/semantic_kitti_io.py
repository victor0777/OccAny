...
def get_kitti_to_common_class_mapping():
 
  kitti_2_common_class_names = [
      "empty", # empty
      "car", #car
      "bicycle", # bicycle
  ...
  common_class_2_idx, _, _ = get_common_classes()
  kitti_2_common_idx = np.zeros(len(kitti_2_common_class_names), dtype=np.int32)
  for kitti_idx, class_name in enumerate(kitti_2_common_class_names):
    kitti_2_common_idx[kitti_idx] = common_class_2_idx[class_name]
  return kitti_2_common_idx


def get_common_classes():
  class2colors = np.array([
            [0, 0, 0, 255], # "empty"
            [100, 150, 245, 255], # "car"
            ...
            [255, 0, 255, 255], # "road"
            # "unknown": [255, 150, 255, 255],
            [75, 0, 75, 255], # "sidewalk"
            # "unknown": [175, 0, 75, 255],
  ...
  return class_2_idx, idx_2_class, class2colors

def get_bravo_to_common_class_mapping():
    bravo_2_common_class_names = [
        "road", # "Road"
        "sidewalk", # "Sidewalk"
        "building", # "Building"
        "unknown", # "Wall"
    ...
    common_classes_2_idx, idx_2_common_class, _ = get_common_classes()
    bravo_idx_2_common_idx = np.zeros(len(bravo_2_common_class_names), dtype=np.int32)
    for bravo_idx, common_class in enumerate(bravo_2_common_class_names):
        bravo_idx_2_common_idx[bravo_idx] = common_classes_2_idx[common_class]
    return bravo_idx_2_common_idx
